...
func generate() -> Dictionary:
	...
	# Trunk
	var trunk_len := randi_range(trunk_len_min, trunk_len_max)
	for y in trunk_len:
		voxels[Vector3(0, y, 0)] = log_type

	# Branches
	var branches_start := randi_range(trunk_len / 3, trunk_len / 2)
	...
	for pos in voxels:
		var rpos = pos + structure.get("offset")
		var v = voxels[pos]
		buffer.set_voxel(v, rpos.x, rpos.y, rpos.z, channel)
		
	structure["voxels"] = buffer
	return structure
